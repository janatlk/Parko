...
- [x] **FE-010.1** Инициализировать проект на основе Vite + React + TypeScript.
  - Настроить alias’ы путей (`@app`, `@features`, `@entities`, `@shared` и т.д.).
- [x] **FE-010.2** Подключить и настроить UI‑библиотеку.
  - Базовая тема (цвета, шрифты), тёмный/светлый режим на будущее.
- [x] **FE-010.3** Настроить ESLint + Prettier.
  - Базовые правила для TS/React.

### FE-011. HTTP‑клиент и работа с API

- [x] **FE-011.1** Настроить axios‑инстанс:
  - Базовый URL (`/api/v1/`).
...
- [x] **FE-011.2** Интегрировать TanStack Query:
...
- [x] **FE-022.1** Реализовать утилиты для проверки ролей:
  - `canEditUsers(user)`, `canEditCars(user)`, `canViewReports(user)` и т.п.
- [x] **FE-022.2** Реализовать HOC/компонент ограничения доступа:
...
- [x] **FE-061.1** Форма создания/редактирования машины:
  - Поля, согласованные с backend‑моделью `Car`.
- [x] **FE-061.2** Ограничение по ролям:
